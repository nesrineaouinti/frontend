...
function RowMenu({ jobId, onDelete, onPromote }) {
  useEffect(() => {
    //this is to fetch the promoted jobs and set their toggle to Promoted on
    axiosInstance
      .get(`jobs/${jobId}/`)
      .then((response) => {
...
export default function JobsTableTest({ onEditJob, onViewDetails }) {
  ...
  const handleNextPage = () => {
    if (page < Math.ceil(jobs.length / rowsPerPage) - 1) {
      setPage(page + 1);
    }
  };
  ...
  const handlePromote = (jobId, promoteStatus) => {
    console.log(jobId);
    axiosInstance
      .patch(`jobs/${jobId}/update/`, { promoted: promoteStatus })
      .then((response) => {
        ...
        >
            ...
            <tbody>
              {jobs
                ...
                .map((jobs) => (
                  <tr key={jobs.id}>
                    <td>
                      <Typography level="body1">{jobs.title}</Typography>
                    </td>
                    <td>
                      <Typography level="body1">
                        {formatDate(jobs.created_at)}
                      </Typography>
                    </td>
                    <td>
                      <Typography level="body1">
                    ...
                    <td>
                      ...
                      >
                        <Button
                          ...
                          size="small"
                          onClick={() => {
                            navigate(`/dashboard/viewdetails/${jobs.id}`);
                          }}
                        >
                          View Details
                        </Button>
                        ...
                        />{" "}
                        {/*bech nab3eth kol jobid lil button mte3eha  w w button ki yenzel 3leha wa7ed traja3li onDelete(jobId) t9oli bara handli delete  */}
                      </Box>
                    </td>
                  </tr>
                ))}
          ...
          <Box sx={{ display: "flex", justifyContent: "flex-end", padding: 2 }}>
            ...
            <IconButton
              ...
              disabled={page >= Math.ceil(jobs.length / rowsPerPage) - 1}
            >

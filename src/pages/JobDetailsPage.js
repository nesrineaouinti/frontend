...
export default function JobDetailsPage(){
  ...
  useEffect(() => {
    axiosInstance.get(`jobs/${jobId}/`) // Make sure to correct the URL based on your API endpoint
      .then(response => {
      ...
      .then(response => {
        const applied = response.data.some(app => app.job === parseInt(jobId));
...
useEffect( ()=> {console.log(job.skills)
          ...
          <Card sx={{p:{md:4}   ,py:{xs:3,md:'auto'}}} >
            <Container component="main" alignItems="left"  >
              <Stack direction='column'>
                ...
                {job.skills && job.skills.split(',').map((skill, index)=> ( //if there is skills then render
          <Chip key={index} variant='outlined' label={skill} color="warning" size="small" />))}
          
        </Stack>
        </Stack>
        <Typography color='primary.main' style={{ fontSize:30,fontWeight:400}}>{job.salary}$ <span style={{color:'#1E1E1E', fontSize:14}}>/month</span></Typography>
        </Stack>
        <Typography style={{color:'#A9B4CD' ,fontSize:16}} >{job.summary}</Typography>
      <Divider/>
        <Typography sx={{pt:3, fontSize:20}}>{job.description}</Typography>
              <Divider/>

              <CardActions sx={{mt:3,justifyContent:'flex-end'}} >
                {alreadyApplied ? <Button disabled variant="contained" >already applied</Button> : <ModalButton job={jobId}/> }
